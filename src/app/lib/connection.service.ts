...
// import {fromEvent, Observable, Subscription, timer} from 'rxjs/op';
import {debounceTime, delay, retryWhen, startWith, switchMap, tap} from 'rxjs/operators';
import * as _ from 'lodash';
import { HttpClient } from '@angular/common/http';
import { Subscription, timer, fromEvent, Observable } from 'rxjs';
      ...
      this.httpSubscription = timer(0, this.serviceOptions.heartbeatInterval)
        .pipe(
          switchMap(() => {
            ...
            return this.http[this.serviceOptions.requestMethod](`${this.serviceOptions.heartbeatUrl}?=t${date}`, {responseType: 'text'})
          }),
          retryWhen(errors =>
            errors.pipe(
              // log error message
